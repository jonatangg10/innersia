...
if(isset($_POST['tipoDeCurso'])){
  if(!empty($_POST['tipoDeCurso'])){
    if(is_numeric($_POST['tipoDeCurso'])){
      ...
      $reporte = $consulta->reportetipoDeCursoConvocatorias($tipoDeCurso);
      ...
      if(is_null($reporte)){
        ...
        $_SESSION['mensaje']="No hay convocatorias en este tipo de curso";
    ...
    $_SESSION['tipo_alert']="danger";
    $_SESSION['mensaje']="Seleccione una tipo de curso";
    echo "<script>location.href='../../views/reportesConvocatorias.php'</script>";
  }
}
...
->setTitle('Reporte convocatorias por tipo de curso')
->setSubject('Reporte')
...
$spread->getActiveSheet()->mergeCells('A3:B3');
$spread->getActiveSheet()->mergeCells('C3:D3');
...
$hoja->setCellValue("A1", "Reporte convocatorias por tipo de curso");
$hoja->setCellValue("A3", "Jornada:");
$hoja->setCellValue("C3", $reporte[0]['tipoCurso']);
...
$fileName="ReporteTipoCursoConvocatorias" . $fecha . ".xlsx";
# Crear un "escritor"
