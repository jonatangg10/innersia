...
if(isset($_POST['fechainicioreporte'])){
  if(!empty($_POST['fechainicioreporte'])){
    ...
    if(!empty($_POST['fechafinreporte'])){
      ...
      $reporte = $consulta->reportefechasContacto($fechainicio,$fechafin);
      ...
      if(is_null($reporte)){
        ...
        $_SESSION['mensaje']="No hay solicitudes de contacto entre estas fechas";
...
->setTitle('Reporte contacto por fechas')
->setSubject('Reporte')
...
$hoja->setCellValue("A1", "Reporte de solicitudes de contacto por fechas");
...
$hoja->setCellValue("D4", "Correo electronico");
$hoja->setCellValue("E4", "Numero de celular");
$hoja->setCellValue("F4", "Fecha de solicitud");
$hoja->setCellValue("G4", "Asunto");
$hoja->setCellValue("H4", "Mensaje");
...
foreach($reporte as $r){
    ...
    $hoja->setCellValue('B'.$n,$r['nombres']);
    $hoja->setCellValue('C'.$n,$r['apellidos']);
    $hoja->setCellValue('D'.$n,$r['correo']);
    $hoja->setCellValue('E'.$n,$r['celular']);
    $hoja->setCellValue('F'.$n,$r['fecha']);
    $hoja->setCellValue('G'.$n,$r['asunto']);
    $hoja->setCellValue('H'.$n,$r['mensaje']);
...
$fileName="ReporteFechasContacto" . $fecha . ".xlsx";
# Crear un "escritor"
