...
if(isset($_POST['fechainicioreporte'])){
  if(!empty($_POST['fechainicioreporte'])){
    ...
    if(!empty($_POST['fechafinreporte'])){
      ...
      $reporte = $consulta->reportefechasComentarios($fechainicio,$fechafin);
      ...
      if(is_null($reporte)){
        ...
        $_SESSION['mensaje']="No hay comentarios entre estas fechas";
...
->setTitle('Reporte comentarios por fechas')
->setSubject('Reporte')
...
$hoja->setCellValue("A1", "Reporte comentarios por fechas");
...
$fileName="ReporteFechasComentarios" . $fecha . ".xlsx";
# Crear un "escritor"
