...
if(isset($_POST['jornada'])){
  if(!empty($_POST['jornada'])){
    if(is_numeric($_POST['jornada'])){
      ...
      $reporte = $consulta->reportecomentariosJornada($jornada);
      // var_dump($jornada);
      // die();
        if(is_null($reporte)){
          $jornadas = array(
          ...
          $_SESSION['mensaje']="No hay comentarios de la jornada " . $jornadas[$jornada];
...
->setTitle('Reporte comentarios por jornadas')
->setSubject('Reporte')
...
$hoja->setCellValue("A1", "Reporte comentarios por jornada");

$hoja->setCellValue("A3", "Genero:");
$hoja->setCellValue("D3", $reporte[0]['nombrejor']);
...
$fileName="ReporteComentariosJornada". $reporte[0]['nombrejor']. $fecha. ".xlsx";
# Crear un "escritor"
