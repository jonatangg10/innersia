...
if(isset($_POST['ficha'])){
  if(!empty($_POST['ficha'])){
    if(is_numeric($_POST['ficha'])){
      $ficha = (isset($_POST['ficha']) ? $_POST['ficha']:NULL);
      $reporte = $consulta->reporteficha($ficha);
    }else{
...
->setTitle('Reporte Aprendices inscritos por ficha')
->setSubject('Reporte')
...
$hoja->setCellValue("A1", "Reporte Aprendices por ficha");

$hoja->setCellValue("A3", "Ficha:");
$hoja->setCellValue("D3", $ficha);
...
$fileName="ReporteFicha". $ficha . "-". $fecha. ".xlsx";
# Crear un "escritor"
